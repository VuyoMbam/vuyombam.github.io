...
The homepage carries invisible schema.org JSON-LD that tells Google the site is the personal site of one named person and links it to the owner's LinkedIn and GitHub profiles, so a search for the name can recognise it. It contains a `WebSite` (site name "Vuyo Mbam", alternate name "Energy with Vuyo Mbam"), a `ProfilePage` and a `Person` (job title, current employer, universities, skills, profile links).

It is generated by `site_utils/structured_data.py` from the `profile:` block at the top of `data/profile.yml`, the current role and education in the same file, and the description in `index.qmd`. To change what it says, edit those, not the HTML. Only put facts there that are already public on the page (no email, phone or address). It updates the `dateModified` on every render.

**Articles** get their own `Article` markup automatically, from the Lua filter `_filters/article-jsonld.lua`, which `posts/_metadata.yml` applies to everything in `posts/`. It uses the article's front matter (`title`, `description`, `date`, optional `date-modified`, `categories` as keywords, `image`) and `data/profile.yml`, and names the same author entity as the homepage. A new article needs nothing extra: just fill in its front matter. Without an `image` it falls back to the site portrait, and without `date-modified` it uses the publish date.

Check it after a deploy with Google's [Rich Results Test](https://search.google.com/test/rich-results) or the [Schema Markup Validator](https://validator.schema.org/). Google decides whether to use the markup and how quickly, so treat it as a strong hint, not a guarantee.
